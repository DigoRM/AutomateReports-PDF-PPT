...
FRANCHISES = ['PizzaGroup España']

AVAILABLE_MONTHS = {
    'PizzaGroup España': ['Marzo 2026', 'Abril 2026', 'Mayo 2026'],
}
...
def export_pdf():
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        // Draw all charts
        const tabsList = ['resumen', 'rankings', 'areas', 'velocidad', 'kbp1', 'kbp2', 'kbp3', 'delivery', 'camcheck'];
        if (D.has_drive_thru) {
            tabsList.splice(8, 0, 'drivethru');
        } else {
            const dtTab = document.getElementById('tab-drivethru');
            if (dtTab) dtTab.style.display = 'none';
            const dtTc = document.getElementById('tc-drivethru');
            if (dtTc) {
                dtTc.style.setProperty('display', 'none', 'important');
                dtTc.classList.remove('active');
            }
        }
    ...
    try:
        ...
        response.headers['Content-Disposition'] = f'attachment; filename=PG_Spain_{period.replace(" ", "_")}_REPORT.pdf'
        return response
    except Exception as e:
...
def export_pptx():
    ...
    tabs_list = ['resumen', 'rankings', 'areas', 'velocidad', 'kbp1', 'kbp2', 'kbp3', 'delivery', 'camcheck']
    try:
        d_val = json.loads(data_json)
        if d_val.get('has_drive_thru'):
            tabs_list.insert(8, 'drivethru')
    except Exception as e:
        print("Error parsing data_json for drivethru filter:", e)
    screenshots = []
    ...
    try:
        ...
        response.headers['Content-Disposition'] = f'attachment; filename=PG_Spain_{period.replace(" ", "_")}_REPORT.pptx'
        return response
