# OptimaReports — Script Completo para Agente de Vibe Coding
## IC México · Report Automation App
...
| Arquivo | Papel |
|---------|-------|
| `OptimaReports_Vibecoding_Script.md` | Este documento — arquitetura, parser, regras |
| `IC_AlphaFranchise_Abril2026_Dashboard.html` | **TEMPLATE BASE** — copiar e adaptar para Jinja2 |
| `IC_Variables_Dinamicas.html` | Referência visual de todas as variáveis com exemplos reais |
...
O app recebe um arquivo Excel por franquia (exportado pelo OptimaReports) e gera
...
- Template: Jinja2 — adaptar `IC_AlphaFranchise_Abril2026_Dashboard.html`
...
│   └── dashboard.html        (copiar IC_AlphaFranchise_Abril2026_Dashboard.html e adaptar)
└── uploads/                  (temporário)
```
...
O arquivo `IC_AlphaFranchise_Abril2026_Dashboard.html` já tem todo o CSS, JS,
...
```javascript
const D = {"franchise":"AlphaFranchise", ... };
```
...
```html
<!-- ANTES -->
<div class="top-meta">IceCream Co. México · AlphaFranchise · 62 Tiendas · Abril 2026</div>

<!-- DEPOIS -->
<div class="top-meta">IceCream Co. México · {{ franchise }} · {{ n_stores }} Tiendas · {{ period }}</div>
```
...
|------------|---------------|-------------|--------------------------------------|--------|
| DeltaFranchise  | 9             | numérico    | ❌ 1 alerta apenas — tratar como sem | ✅     |
| BetaFranchise     | 20            | `[g]`       | ✅ 9 lojas · 10 alertas DT           | ❌     |
| GammaFranchise       | 17            | numérico    | ✅ 10 lojas · **154 alertas DT**     | ✅     |
| AlphaFranchise | 62–63         | `[z]`       | ❌ zero alertas DT                   | ✅     |

**Dados reais Drive Thru — GammaFranchise Abril 2026 (para teste):**
```
...
# parser.py
FRANCHISES = {
    'DeltaFranchise':  {'expected_stores': 9,  'has_drive_thru': False},
    'BetaFranchise':     {'expected_stores': 20, 'has_drive_thru': True},
    'GammaFranchise':       {'expected_stores': 17, 'has_drive_thru': True},
    'AlphaFranchise': {'expected_stores': 63, 'has_drive_thru': False},
}

AVAILABLE_MONTHS = {
    'DeltaFranchise':  ['Marzo 2026', 'Abril 2026'],
    'BetaFranchise':     ['Abril 2026'],   # sem dados em Março
...
Row indices FIXOS para todos os arquivos OptimaReports com este template.
...
| Row | ID OptimaReports | Alerta | Área |
|-----|----------|--------|------|
...
<head>
<meta charset="UTF-8">
<title>OptimaReports · Generar Reporte IC México</title>
...
<body>
<div class="logo">OptimaReports<span>.ai</span></div>
<div class="subtitle">DA07QUMX · IceCream Co. México · Report Generator</div>
...
```python
# GammaFranchise tem [D] alerts no top8; AlphaFranchise não tem DT → [D] nunca aparecem
...
// "IC ARAGON [z]" → "IC ARAGON"
// "IC Hampton [g]" → "IC Hampton"
...
- [ ] `IC_AlphaFranchise_Abril2026_Dashboard.html` = template base → adaptar para Jinja2
- [ ] `IC_Variables_Dinamicas.html` = referência de todas as variáveis com exemplos
...
- [ ] BetaFranchise sem dados em Março — não bloquear, apenas avisar
...
- [ ] Testar com planilha da GammaFranchise para validar Drive Thru real
- [ ] PDF via `window.print()` — já implementado no template

---

*OptimaReports · IC México Report Automation · Script v3.0 · Maio 2026*
